...
# This is for length 4
# No writing to db for now
...
imunicode = {'R':'🟥','R':'❌','G':'🟩','G':'✅','B':'🟦','B':'🔵','B':'☂️','Y':'🟨','Y':'↔️'}
...
def test_for_yellows_conly(ss,sconsonants,tconsonants):
    ...
    for i in checklist: # i is from test, so pos in ytconsonants
        if ytconsonants[i] != ['']: # skip pure vowels
            for j in checklist: # j is from secret, so pos in ysconsonants
                #st.write(i,j,ytconsonants[i],ysconsonants[j],accountedc)
                if j not in accountedc and (set(ytconsonants[i]) & set(ysconsonants[j])):
                    #st.write("found in union")
                    ys.append(i)
                    accountedc.append(j) # j is now matched, so don't match it again
                    #st.write("appending %d to ys due to consonant match" % i)
                    break
    uniq_ys = set(ys)
    #st.write(set(ys),uniq_ys)
